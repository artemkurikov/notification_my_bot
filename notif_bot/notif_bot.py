...
def reg(message):
    req_check_user = req_get(f'http://{ip}:{port}/user/{message.from_user.id}')
...
#        bot.register_next_step_handler(msg, get_gmt)
    ...
    if re_search(r'\\set \d\d:\d\d (\d\d\.\d\d.\d\d|today|) \w.*', message.text):
        ...
        req_add_notif = req_post(f'http://{ip}:{port}/new_notif', headers={'Content-Type': 'application/json'}, data=js_dumps(notif_list))
...
def get_gmt(message):
    if re_search(r'^(g|G)(m|M)(t|T)\s*(\+|\-)\s*(1[0-2]|0[0-9]|[0-9]$)', message.text):
        ...
        req_reg = req_post(f'http://{ip}:{port}/user_regist', headers={'Content-Type': 'application/json'}, data=js_dumps(user_data))
